...
public class ColorController extends AbstractController {
	 ...
	 * @return
	 */
	@RequestMapping(value = { "/color/queryByList" }, method = RequestMethod.POST)
	@NotNull(value = "token")
	@ResponseBody
	public ReturnBody getColorByList(@RequestBody ParamsBody paramsBody,
			HttpServletRequest request, HttpServletResponse response) {
		ReturnBody rbody = new ReturnBody();
		...
		if (user != null) {
			...
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("clientId", clientId);
			List<Color> list = colorService.getColorPageByCondition(map);
			rbody.setData(list);
			rbody.setStatus(ResponseState.SUCCESS);
			return rbody;
		} else {
