...
app.factory("BaseService",['$http',function($http){
    return{
        post : function(url,params){
            ...
            paramsBody.timestamp = new Date().getTime();
...
app.service("printService",function(BaseService){
	this.getData = function(str,orderId){
		var params = {"orderId":orderId}
		BaseService.post("../rest/statistics/printOrder",params).then(function(data){
			var details = JSON.stringify(data.data.data)
		
			window.localStorage.setItem("details",details)
			loadPrintDocument(str,orderId)
		})
	}
	this.printPage = function(str,orderId){
		loadPrintDocument(str,orderId)
	};
	function loadPrintDocument(str,orderId){
		
		$("body").append(components.messageBox("正在准备打印数据⋯⋯"));
	    $("#printMessageBox").css("opacity", 0);
	    $("#printMessageBox").animate({opacity:1}, 300, function() { addIframeToPage(str,orderId); });
	};
	function addIframeToPage(str,orderId){

	      var url = "./view/template/print.html?type="+str+"&orderId="+orderId;
